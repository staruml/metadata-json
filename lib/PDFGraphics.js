...
define(function (require, exports, module) {
    ...
    function isCJK(ch) {
        ...
        if (0x2E80<=c && c<=0x2EFF) return true;
        if (0x2F00<=c && c<=0x2FDF) return true;
        if (0x2FF0<=c && c<=0x2FFF) return true;
        if (0x3000<=c && c<=0x303F) return true;
        if (0x31C0<=c && c<=0x31EF) return true;
        if (0x3200<=c && c<=0x32FF) return true;
        if (0x3400<=c && c<=0x4DBF) return true;
        if (0x4E00<=c && c<=0x9FFF) return true;
        if (0x20000<=c && c<=0x2A6DF) return true;
        // Japanese Chars
        if (0x3000<=c && c<=0x303f) return true;
        if (0x3040<=c && c<=0x309f) return true;
        if (0x30a0<=c && c<=0x30ff) return true;
        if (0xff00<=c && c<=0xff9f) return true;
        // Cryllic
        if (0x0400<=c && c<=0x04FF) return true;
        if (0x0500<=c && c<=0x052F) return true;
        if (0x2DE0<=c && c<=0x2DFF) return true;
        if (0xA640<=c && c<=0xA69F) return true;
        return false;
    }

    function parseCJK(str) {
        var list = [];
        if (str.length > 0) {
            var s = '';
            var cjk = isCJK(str[0]);
    ...
    Canvas.prototype._setFont = function (font) {
        ...
        var _fs = _fontSet[font.face.trim()];
        if (!Array.isArray(_fs)) {
            _fs = _fontSet["Helvetica"]; // Default Font
        }
        ...
        */
        ...
        case Graphics.FS_NORMAL:
            this.context.font(_fs[0]);
            break;
        case Graphics.FS_BOLD:
            this.context.font(_fs[1]);
            break;
        case Graphics.FS_ITALIC:
            this.context.font(_fs[2]);
            break;
        case Graphics.FS_BOLD_ITALIC:
            this.context.font(_fs[3]);
            break;
        }
    };
    ...
    Canvas.prototype.wordWrap = function (text, width) {
        ...
        if (lines.length > 0) {
            ...
            for (var i = 1, len = lines.length; i < len; i++) {
                var t = term + " " + lines[i],
                    w = this.__widthOfStringCJK(t, this.font.size);
                if (w <= width) {
                    term = t;
                } else {
                    result.push(term);
    ...
    /**
     * Compute width of CJK (Chinese-Japanese-Korean) string
     * @private
     */
    Canvas.prototype.__widthOfStringCJK = function (text, fontSize) {
        var terms = parseCJK(text);
        var width = 0;
        var CJKFont = { face: "System", size: this.font.size, style: this.font.style };
        for (var i = 0, len = terms.length; i < len; i++) {
            var term = terms[i];
            if (isCJK(term[0])) {
                this._setFont(CJKFont);
    ...
    /**
     * Draw CJK (Chinese-Japanese-Korean) text with Default System Font
     * @private
     */
    Canvas.prototype.__textOutCJK = function (text, x, y, options) {
        var terms = parseCJK(text);
        var CJKFont = { face: "System", size: this.font.size, style: this.font.style };
        var _x = x,
            _y = y;
        ...
        if (options.align) {
            var width = this.__widthOfStringCJK(text, this.font.size);
    ...
    Canvas.prototype.textOut2 = function (rect, text, horizontalAlignment, verticalAlignment, rotate, wordWrap, underline) {
        ...
        if (rotate === true) {
            ...
            this.__textOutCJK(text, 0, 0, options);
        } else {
            switch (verticalAlignment) {
            case Graphics.AL_TOP:
                baseY = rect.y1;
            ...
            if (wordWrap) {
                ...
                for (var i = 0, len = lines.length; i < len; i++) {
                    ...
                    w = this.__widthOfStringCJK(lines[i], this.font.size);
                    if (w > options.width) {
                        options.width = w;
                    }
                    this.__textOutCJK(lines[i], baseX, baseY, options);
                    baseY = baseY + this.font.size;
                }
            } else {
                // To avoid that a part of text is not rendered
                w = this.__widthOfStringCJK(text, this.font.size);
                if (w > options.width) {
                    options.width = w;
                }
                this.__textOutCJK(text, baseX, baseY, options);
            }
        }
        this.context.restore();
        this.restoreTransform();
    ...
    Canvas.prototype.textExtent = function (text, wordWrapWidth) {
        ...
        if (wordWrapWidth) {
            var lines = this.wordWrap(text, wordWrapWidth),
                widths = _.map(lines, function (line) { return self.__widthOfStringCJK(line, self.font.size); });
            sz.x = _.max(widths);
            sz.y = this.font.size * lines.length;
        } else {
            sz.x = this.__widthOfStringCJK(text, this.font.size);
            sz.y = this.font.size;
        }
        return sz;
    };
